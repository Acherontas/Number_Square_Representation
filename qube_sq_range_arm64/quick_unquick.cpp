...
int quick_unquick::unquick(long long int xbi,long long int xtout){
    long double xpik=0;
    long long int xp=0;
    xpik=sqrt(xbi);
    ...
    for(i=(xp+1);i<=xbi;i+=pvt){
       ...
       if((i<xbi) && (xbi<mpos) && (mpos>xbi)){
                                    seira=xbi-i;
                                    cout<<xts << " :: " << i << " ... " <<  xbi << " (@ " << seira << " ) "  << " ... " << mpos << "\n"; seira=0;
                                    goto A;
                                  }
       A:cout<<"";
    }
...
int quick_unquick::quick(long long int xb){
    long double xpik=0;
    long long int xp=0;
    xpik=sqrt(xb);
    long double fractpart,intpart;
